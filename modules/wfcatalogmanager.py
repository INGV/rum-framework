...
class WFCatalogCollector():
    ...
    def __init__(self, parsedargs, config, mongo, log):
        """
    ...
    def _setOptions(self):
        ...
        for key in self.parsedargs:
            default_options[key] = self.parsedargs[key]

        self.args = default_options

        # Check if there is a single input method
        nInput = 6 - [self.args['date'], self.args['file'], self.args['dir'], self.args['list'], self.args['past'],
                      self.args['glob']].count(None)
        if nInput == 0:
        ...
        if self.config['MONGO']['ENABLED']:
            if not self.mongo._connected:
                try:
                    ...
                    self.log.critical("Could not establish connection to the database");
                    # sys.exit(0)
                    return
            else:
        ...
        for file in self.files:
            ...
            for documents in self.mongo.getDailyFilesById(file):
                ...
                if self._getFullPath(documents["fileId"]) not in self.files:
                    ...
                    # print(" _getFullPath file is:")
                    # print(self._getFullPath(documents["fileId"]))

            # Remove the document
            # print("get doc by filename")
            document = self.mongo.getDocumentByFilenameOne(file)
            # print(document)
            ...
            if document is not None:
                ...
                self.log.info("NOT removed: document doesn't exist anymore %s." % file)
    ...
    def _collectFilesFromDate(self, date):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            directory = os.path.join(self.config['ARCHIVE_ROOT'], year, jday)
            collectedFiles = [os.path.join(directory, f) for f in os.listdir(directory) if
    ...
    def _getFiles(self):
        ...
        # Raise if an invalid input directory is given
            ...
            if not os.path.isdir(self.args['dir']):
                raise Exception("Input is not a valid directory on the file system." + self.args['dir'])

            # Collect all the files (recursively) from a directory and add them
            self.files = [os.path.join(root, f) for root, dirs, files in os.walk(self.args['dir']) for f in files if
        ...
        # Raise on no input
            ...
            raise Exception(
                "Input is empty. Use --dir, --file, or --list to specify a directory, file, or list of files to process.")

    def _validateFilters(self):
        """
        WFCatalogCollector._validateFilters
        > sanity checks the filters in the self.config.json
        """
        if len(self.config['FILTERS']['WHITE']) == 0:
    ...
    def _filterFiles(self):
        ...
        # print(" before self.totalFiles") 
        self.totalFiles = len(self.files)
        if self.totalFiles == 0:
            self.log.info("No files for processing: doing nothing.");
            # sys.exit(0)
            return
    ...
    def _getFullPath(self, file):
        ...
        > for file basename and directory stucture (self.config)
        """
    ...
    def _isNewDocument(self, file):
        ...
        if not self.config['MONGO']['ENABLED']:
            return True
        elif self.config['MONGO']['ALLOW_DOUBLE']:
            return True
        else:
            # my_result = self.mongo.getDocumentByFilename(file).count()
            # my_result = self.mongo.getDocumentByFilename(file)
            # print('isNewDoc : myresult:')
            # print(my_result)
            exist_file = self.mongo.getDocumentByFilenameOne(file)
            if exist_file:
                print("ERROR document exist in mongo - return false")
                return False

            else:
                print("is new mongo document: OK")
                return True
    ...
    def _callObsPyMetadata(self, files, start, end, granule):
        ...
        signal.alarm(self.config['PROCESSING_TIMEOUT'])
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                if granule == 'daily':
                    metadata = MSEEDMetadata(files, starttime=start, endtime=end, add_flags=self.args['flags'],
        ...
        if self.args['csegs'] and not qc_metadata_daily['cont']:
            for segment in documents['daily']['c_segments']:
                try:
                    ...
                    self.log.error("Could not store continuous segment to database")
            self.log.info(
                "Succesfully stored %d continuous segment(s) to database" % len(documents['daily']['c_segments']))
    ...
    def _getFilename(self, stats):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            ...
            filename = ".".join(
                [stats['network'], stats['station'], stats['location'], stats['channel'], stats['dtype'], stats['year'],
                 stats['jday']])
    ...
    def _getFileDirectory(self, stats):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            ...
            filepath = os.path.join(stats['year'], stats['network'], stats['station'],
                                    stats['channel'] + "." + stats['dtype'], self._getFilename(stats))

        else:
            raise Exception("Unknown directory structure in self.config (expected ODC or SDS)")

        return os.path.join(self.config['ARCHIVE_ROOT'], filepath)
